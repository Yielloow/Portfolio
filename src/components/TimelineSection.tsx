...
export default function TimelineSection() {
  ...
  const typeLabels: Record<TimelineItem["type"], string> = {
    education: t("timeline.type.education"),
    work: t("timeline.type.work"),
    project: t("timeline.type.project"),
    other: t("timeline.type.other"),
  };
    ...
    <section id="timeline" className="py-24 px-6">
      <div className="max-w-4xl mx-auto">
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5 }} className="mb-16">
          <p className="text-primary font-heading text-sm tracking-[0.2em] uppercase mb-3">{t("timeline.label")}</p>
          <h2 className="font-heading text-3xl md:text-4xl font-bold">
            {t("timeline.title_start")}<span className="text-gradient">{t("timeline.title_highlight")}</span>
          </h2>
        </motion.div>
                  ...
                  <div className="glass-card rounded-xl p-5 hover:glow-accent transition-shadow duration-300">
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <span className="text-xs text-muted-foreground">
                        {formatDate(item.startDate)} — {item.endDate === "present" ? t("timeline.present") : formatDate(item.endDate)}
                      </span>
                    </div>
